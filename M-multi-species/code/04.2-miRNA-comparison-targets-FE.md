...
- [5 Is correlation direction related to binding
  ...
  - [5.1 Chisq – IGNORE](#51-chisq--ignore)
  - [5.2 miRNA-level proportion tests](#52-mirna-level-proportion-tests)
  - [5.3 Plot](#53-plot)
...
```
    ...
    ## ℹ Row 1724 of `y` matches multiple rows in `x`.
...
```
    ...
    ## ℹ Row 683 of `y` matches multiple rows in `x`.
    ## ℹ If a many-to-many relationship is expected, set `relationship =
    ##   "many-to-many"` to silence this warning.
...
```
    ...
    ## ℹ Row 1256 of `y` matches multiple rows in `x`.
    ## ℹ If a many-to-many relationship is expected, set `relationship =
    ##   "many-to-many"` to silence this warning.
...
```
    ...
    ## [14] "lipid metabolic process"                               
    ## [15] "protein-containing complex assembly"                   
    ## [16] "regulation of DNA-templated transcription"             
    ## [17] "cilium organization"                                   
    ## [18] "circulatory system process"                            
    ## [19] "cytoskeleton organization"                             
    ## [20] "membrane organization"                                 
    ## [21] "microtubule-based movement"                            
    ## [22] "nervous system process"                                
    ## [23] "nucleobase-containing small molecule metabolic process"
    ## [24] "programmed cell death"                                 
    ## [25] "reproductive process"
...
```
    ...
    ## [1] ""                                    "signaling"                          
    ## [3] "anatomical structure development"    "protein-containing complex assembly"
...
```
    ...
    ##  [8] "carbohydrate derivative metabolic process"             
    ##  [9] "cell motility"                                         
...
```
    ...
    ## 2                                                                                                                                                                                                                                                            tRNA metabolic process;amino acid metabolic process;nucleobase-containing small molecule metabolic process
    ...
    ## 24                                                                                                                                                                                                                                                                                               cell adhesion;nervous system process;extracellular matrix organization
...
Note the below plots are showing *unique* interactions (one per
miRNA-mRNA pair that is predicted to interact). They do *not* show
multiple counts if an miRNA is predicted to bind in multiple location on
a given mRNA.
...
``` r
stacked_bar_simple <- function(df, species_color, species_name, ymin = NULL, ymax = NULL, cons_asterisk = FALSE, max_targets = NULL) {
  # Label correlation direction
  df_labeled <- df %>%
    mutate(
      correlation_direction = case_when(
  ...
  # Count mRNA targets per miRNA & direction
  plot_data <- df_labeled %>%
    group_by(given_miRNA_name, correlation_direction) %>%
    summarise(n_mRNA = n_distinct(mRNA), .groups = "drop")
  # Order miRNAs by number of targets
  miRNA_order <- plot_data %>%
    group_by(given_miRNA_name) %>%
    summarise(total = sum(n_mRNA), .groups = "drop") %>%
    arrange(desc(total))
  
  # Filter to miRNAs below max_targets threshold if specified
  if (!is.null(max_targets)) {
    miRNA_order <- miRNA_order %>%
      filter(total < max_targets)
  }
  
  miRNA_order <- miRNA_order %>%
    pull(given_miRNA_name)
  
  # Filter plot_data to only include miRNAs that passed the threshold
  plot_data <- plot_data %>%
    filter(given_miRNA_name %in% miRNA_order) %>%
    mutate(given_miRNA_name = factor(given_miRNA_name, levels = miRNA_order)) %>%
    mutate(correlation_direction = factor(correlation_direction, levels = c("negative", "positive"))) %>%
    arrange(given_miRNA_name, correlation_direction)
  
  # Asterisks for conserved miRNAs
  bar_heights_pos <- plot_data %>%
    group_by(given_miRNA_name) %>%
  ...
  asterisk_df <- bar_heights_pos %>%
    filter(given_miRNA_name %in% conserved) %>%
    mutate(label = "*", y_pos = pos_height + offset)
  
  p <- ggplot(plot_data, aes(x = given_miRNA_name, y = n_mRNA)) +
    geom_col(aes(fill = correlation_direction), position = position_stack(reverse = TRUE)) +
    scale_fill_manual(
      values = c(
        negative = "#d7191c",
...
``` r
# Basic plotting, using all same color, for manuscript
p1 <- stacked_bar_simple(Apul_FA_df, "#408EC6", "A. pulchra", ymin=0 , ymax=250, cons_asterisk=TRUE)
p2 <- stacked_bar_simple(Peve_FA_df, "#408EC6", "P. evermanni", ymin=0 , ymax=250, cons_asterisk=TRUE)
p3 <- stacked_bar_simple(Ptuh_FA_df, "#408EC6", "P. tuahiniensis", ymin=0 , ymax=250, cons_asterisk=TRUE)
p1
```
...
``` r
ggsave("../output/04.2-miRNA-comparison-targets-FE/Apul_miRNA-mRNA_interactions.png", p1, width = 12, height = 8, dpi = 300)
ggsave("../output/04.2-miRNA-comparison-targets-FE/Peve_miRNA-mRNA_interactions.png", p2, width = 12, height = 8, dpi = 300)
ggsave("../output/04.2-miRNA-comparison-targets-FE/Ptuh_miRNA-mRNA_interactions.png", p3, width = 12, height = 8, dpi = 300)

# Also plot stuff with <100 counts for better visibility of low counts
stacked_bar_simple(Apul_FA_df, "#408EC6", "A. pulchra", ymin=0 , ymax=25, cons_asterisk=TRUE, max_targets=25)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-23-7.png)<!-- -->

``` r
stacked_bar_simple(Peve_FA_df, "#408EC6", "P. evermanni", ymin=0 , ymax=25, cons_asterisk=TRUE, max_targets=25)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-23-8.png)<!-- -->

``` r
stacked_bar_simple(Ptuh_FA_df, "#408EC6", "P. tuahiniensis", ymin=0 , ymax=25, cons_asterisk=TRUE, max_targets=25)
```
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-23-10.png)<!-- -->

``` r
stacked_opacity_bar(Peve_FA_df[Peve_FA_df$conservation=="conserved",], "#1E2761", ymin=-50, ymax=250)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-23-11.png)<!-- -->

``` r
stacked_opacity_bar(Ptuh_FA_df[Ptuh_FA_df$conservation=="conserved",], "#7A2048", ymin=-50, ymax=250)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-23-12.png)<!-- -->
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-24-1.png)<!-- -->

``` r
stacked_opacity_bar_t(Peve_FA_df, "#1E2761", xmin=-200 , xmax=250, cons_asterisk=TRUE)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-24-2.png)<!-- -->

``` r
stacked_opacity_bar_t(Ptuh_FA_df, "#7A2048", xmin=-200 , xmax=250, cons_asterisk=TRUE)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-24-3.png)<!-- -->

Can’t add a log scale to stacked bar plots, it would cause distortion.
Instead, make an option where the bars are not stacked

``` r
log_bar_simple <- function(df, species_color, species_name, ymin = NULL, ymax = NULL, cons_asterisk = FALSE) {
  ...
  # Order miRNAs by total interactions
  miRNA_order <- plot_data %>%
    group_by(given_miRNA_name) %>%
    summarise(total = sum(n_mRNA)) %>%
    arrange(desc(total)) %>%
    pull(given_miRNA_name)
  
  plot_data <- plot_data %>%
    mutate(given_miRNA_name = factor(given_miRNA_name, levels = miRNA_order),
           correlation_direction = factor(correlation_direction, 
  ...
  # Sum of both neg + pos for placement of asterisk
  asterisk_df <- plot_data %>%
    group_by(given_miRNA_name) %>%
    summarise(total_height = sum(n_mRNA)) %>%
    filter(given_miRNA_name %in% conserved) %>%
    mutate(label = "*", y_pos = total_height * 1.1)   # 10% above tallest bar
  ...
  p <- ggplot(plot_data, aes(x = given_miRNA_name, y = n_mRNA,
    ...
    scale_fill_manual(
      values = c(
        negative = "#d7191c",      # red
        positive = species_color    # species color
      ),
      breaks = c("negative", "positive")
    ) +
    scale_y_log10(
    ...
    labs(
      title = paste0(species_name, " miRNA–mRNA Interactions"),
      x = "miRNA",
      y = "Number of mRNA Targets (log10)",
      fill = "Correlation"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
  # y-limits if provided
  if (!is.null(ymin) & !is.null(ymax)) {
    p <- p + coord_cartesian(ylim = c(ymin, ymax))
  }
  
  # Add asterisks if requested
  if (cons_asterisk) {
    p <- p + geom_text(
      data = asterisk_df,
      aes(x = given_miRNA_name, y = y_pos, label = label),
...
log_bar_simple(Apul_FA_df, "#408EC6", "A. pulchra", ymin=1, ymax=1000, cons_asterisk=TRUE)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-25-1.png)<!-- -->

``` r
log_bar_simple(Peve_FA_df, "#408EC6", "P. evermanni", ymin=1, ymax=1000, cons_asterisk=TRUE)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-25-2.png)<!-- -->

``` r
log_bar_simple(Ptuh_FA_df, "#408EC6", "P. tuahiniensis", ymin=1, ymax=1000, cons_asterisk=TRUE)
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-25-3.png)<!-- -->
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-26-4.png)<!-- -->

``` r
region_pie(Peve_FA_conserved, "#1E2761")
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-26-5.png)<!-- -->

``` r
region_pie(Ptuh_FA_conserved, "#7A2048")
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-26-6.png)<!-- -->

``` r
region_pie(Apul_FA_df[sign(Apul_FA_df$PCC.cor) > 0,], "#408EC6")
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-26-7.png)<!-- -->

``` r
region_pie(Peve_FA_df[sign(Peve_FA_df$PCC.cor) > 0,], "#1E2761")
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-26-8.png)<!-- -->

``` r
region_pie(Ptuh_FA_df[sign(Ptuh_FA_df$PCC.cor) > 0,], "#7A2048")
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-26-9.png)<!-- -->
...
``` r
print("Apul: range, mean, and median # sites/target")
```

    ## [1] "Apul: range, mean, and median # sites/target"
...
``` r
print("Peve: range, mean, and median # sites/target")
```

    ## [1] "Peve: range, mean, and median # sites/target"
...
``` r
print("Ptuh: range, mean, and median # sites/target")
```

    ## [1] "Ptuh: range, mean, and median # sites/target"
...
Save dfs showing instances of multiple binding sites between single
miRNA-mRNA pairs for supplementary files

``` r
write.csv(filter(Apul_binding_sum[,1:7], total_binding_sites>1), "../output/04.2-miRNA-comparison-targets-FE/Apul_multibinding.csv", row.names = FALSE, quote = FALSE)
write.csv(filter(Peve_binding_sum[,1:7], total_binding_sites>1), "../output/04.2-miRNA-comparison-targets-FE/Peve_multibinding.csv", row.names = FALSE, quote = FALSE)
write.csv(filter(Ptuh_binding_sum[,1:7], total_binding_sites>1), "../output/04.2-miRNA-comparison-targets-FE/Ptuh_multibinding.csv", row.names = FALSE, quote = FALSE)
```
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-30-1.png)<!-- -->
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-30-2.png)<!-- -->
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-30-3.png)<!-- -->
...
However, while the above test is a good indication there may be a
relationship between correlation direction and species, our data doesn’t
meet the assumptions for a Chisq test of independence. The test assumes
observations are independent. However, a single miRNA may bind to
multiple mRNA, a single mRNA may be targeted by multiple miRNA, and a
single miRNA may even target multiple sites within the same *region* of
the same mRNA. Additionally, since the test uses raw counts, it’s
potentially skewed/dominated by the small number of miRNA with a very
high number of targets (e.g., miR-100). Because of these, we should try
another approach.

## 5.2 miRNA-level proportion tests
...
```
    ...
    ## # A tibble: 3 × 8
    ##   species n_miRNAs mean_prop_CDS median_prop_CDS mean_prop_3UTR_UTR
    ##   <fct>      <int>         <dbl>           <dbl>              <dbl>
    ## 1 Apul          39         0.789           0.812              0.567
    ## 2 Peve          41         0.225           0                  0.513
    ## 3 Ptuh          37         0.298           0                  0.472
    ## # ℹ 3 more variables: median_prop_3UTR_UTR <dbl>, mean_prop_pos <dbl>,
    ## #   median_prop_pos <dbl>
...
```
    ...
    ## 
    ## data:  prop_CDS by species
    ## Kruskal-Wallis chi-squared = 36.48, df = 2, p-value = 1.198e-08
...
```
    ...
    ## 
    ## data:  mir_props$prop_CDS and mir_props$species 
    ## 
    ##      Apul    Peve
    ## Peve 6.1e-07 -   
    ## Ptuh 1.4e-06 0.47
...
```
    ...
    ##  Kruskal-Wallis rank sum test
    ## 
    ## data:  prop_3UTR_among_UTR by species
    ## Kruskal-Wallis chi-squared = 2.2883, df = 2, p-value = 0.3185
...
```
    ...
    ##  Pairwise comparisons using Wilcoxon rank sum test with continuity correction 
    ## 
    ## data:  mir_utr_only$prop_3UTR_among_UTR and mir_utr_only$species 
    ## 
    ##      Apul Peve
    ## Peve 0.36 -   
    ## Ptuh 0.36 0.96
...
```
    ...
    ##  Kruskal-Wallis rank sum test
    ## 
    ## data:  prop_pos by species
    ## Kruskal-Wallis chi-squared = 6.5386, df = 2, p-value = 0.03803
...
```
    ...
    ##  Pairwise comparisons using Wilcoxon rank sum test with continuity correction 
    ## 
    ## data:  mir_props$prop_pos and mir_props$species 
    ## 
    ##      Apul  Peve 
    ## Peve 0.059 -    
    ## Ptuh 0.056 0.670
...
So there is a significant difference in proportion of target sites in
the CDS across species (Kruskal-Wallis, p = 1.198e-08), with A. pulchra
showing a higher proportion of CDS sites than either of the other two
species (Wilcoxon rank sum test w cont. correction. Apul-Peve: FDR p =
6.1e-07; Apul-Ptuh: FDR p = 1.4e-06), though P. evermanni and P.
tuahiniensis did not significantly differ (Peve-Ptuh: FDR p = 0.47).

There is also an indication that coexpression direction may differ
across species (Kruskal-Wasllis, p = 0.038), with Apul miRNAs showing
more positive target coexpression, but the trend does not reach
significance in FDR-corrected pairwise tests (Wilcoxon rank sum test
with cont. correction. Apul-Peve: FDR p = 0.059; Apul-Ptuh: FDR p =
0.056; Peve-Ptuh: FDR p = 0.670).

Within-species paired Wilcoxon

Does direction depend on region (CDS vs collapsed-UTR), within each
species?
...
```
    ...
    ## Within-species paired CDS vs UTR comparisons
...
```
    ...
    ## 
    ## --- Apul ---
    ## n miRNAs targeting both regions: 26 
    ## CDS > UTR: 13  | UTR > CDS: 13  | tied: 0 
    ## Mean prop_pos in CDS: 0.693  | Mean prop_pos in UTR: 0.673 
    ## 
    ##  Wilcoxon signed rank exact test
    ## 
    ## data:  d$prop_pos_CDS and d$prop_pos_UTR
    ## V = 189, p-value = 0.7454
...
```
    ...
    ## 
    ## --- Peve ---
    ## n miRNAs targeting both regions: 8 
    ## CDS > UTR: 4  | UTR > CDS: 4  | tied: 0 
    ## Mean prop_pos in CDS: 0.507  | Mean prop_pos in UTR: 0.493 
    ## 
    ##  Wilcoxon signed rank exact test
    ## 
    ## data:  d$prop_pos_CDS and d$prop_pos_UTR
...
```
    ...
    ## 
    ## --- Ptuh ---
    ## n miRNAs targeting both regions: 9 
    ## CDS > UTR: 9  | UTR > CDS: 0  | tied: 0 
    ## Mean prop_pos in CDS: 0.471  | Mean prop_pos in UTR: 0.295 
    ## 
    ##  Wilcoxon signed rank exact test
    ## 
    ## data:  d$prop_pos_CDS and d$prop_pos_UTR
...
Within-species comparisons show that, in Ptuh, target coexpression
direction and target binding region are significantly related (paired
Wilcoxon, p = 0.0039). In all Ptuh miRNAs that target both CDS and UTR
regions, the CDS targets are significantly more frequently *positively*
coexpressed. There is no such relationship for Apul (p = 0.745) or Peve
(p = 0.641).

This is a notable difference from the preliminary chisq test used above,
which showed Ptuh CDS targets were more likely to to have *negative
correlation*. Tests based on miRNAs’ positive proportions show the exact
opposite! This difference is likely because the Chisq test, which used
raw correlation counts instead of miRNA-level proportions, was dominated
by a few miRNA with many many largely-negative targets (e.g., miR-100).
We should **discard that Chisq test’s results** in favor of this more
robust approach.

## 5.3 Plot

``` r
# Make sure species is a factor with the desired order
mir_props <- mir_props %>%
  mutate(species = factor(species, levels = c("Apul","Peve","Ptuh")))

# Set colors
species_pal    <- c(Apul = "#408EC6", Peve = "#1E2761", Ptuh = "#7A2048")
species_labels <- c(Apul = "A. pulchra",
                    Peve = "P. evermanni",
                    Ptuh = "P. tuahiniensis")

# ============================================================================
# FIGURE 1 — Region distribution across species (per-miRNA proportion in CDS)
# ============================================================================

# Filter out any miRNA with fewer than 5 predicted target sites. A low total # of sites
# may by chance have 0 CDS sites -> 0% CDS sites, could artificially deflate the proportion 
# comparison. Essentially, trying to reduce weighting by site #.
mir_props_filt <- mir_props %>% filter(n_sites >=5)

fig1 <- ggplot(mir_props_filt, aes(species, prop_CDS, fill = species)) +
  ...
  labs(x = NULL, y = "Per-miRNA proportion of sites in CDS",
       title = "Region distribution: CDS vs UTR") +
  theme(axis.text.x = element_text(face = "italic"))

ggsave("../output/04.2-miRNA-comparison-targets-FE/region_distribution.png",
       fig1, width = 4.5, height = 4, dpi = 300, bg = "white")

# ============================================================================
# FIGURE 2 — Coexpression direction across species
# ============================================================================

fig2 <- ggplot(mir_props, aes(species, prop_pos, fill = species)) +
  ...
  scale_y_continuous(limits = c(0, 1.02),
                     breaks = seq(0, 1, 0.25),
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of positively-coexpressed sites",
       title = "Coexpression direction across species") +
  theme(axis.text.x = element_text(face = "italic"))

ggsave("../output/04.2-miRNA-comparison-targets-FE/direction.png",
       fig2, width = 4.5, height = 4, dpi = 300, bg = "white")

# ============================================================================
# FIGURE 3 — Within-species paired CDS vs UTR comparison
# Build paired data from all_spec; restrict to miRNAs with >= 5 sites in BOTH
# CDS and UTR-collapsed regions (matches the Wilcoxon paired test exactly).
# ============================================================================

paired_data <- all_spec %>%
  mutate(region2 = ifelse(region == "CDS", "CDS", "UTR")) %>%
  group_by(species, given_miRNA_name, region2) %>%
  summarise(n        = n(),
            n_pos    = sum(PCC.dir_bin),
            prop_pos = n_pos / n,
            .groups  = "drop") %>%
  ...
  pivot_wider(names_from  = region2,
              values_from = c(n, n_pos, prop_pos)) %>%
  filter(!is.na(prop_pos_UTR) & !is.na(prop_pos_CDS)) %>%
  pivot_longer(cols      = c(prop_pos_UTR, prop_pos_CDS),
               names_to  = "region",
               values_to = "prop_pos") %>%
  mutate(region  = factor(sub("prop_pos_", "", region), levels = c("UTR","CDS")),
         species = factor(species, levels = c("Apul","Peve","Ptuh")))

fig3 <- ggplot(paired_data,
               aes(region, prop_pos, group = given_miRNA_name, colour = species)) +
  geom_hline(yintercept = 0.5, linetype = "dashed",
             colour = "grey55", linewidth = 0.4) +
  ...
  scale_y_continuous(limits = c(0, 1.02),
                     ...
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of positively-coexpressed sites",
       title    = "Within-species CDS vs UTR comparison",
       subtitle = "Each line is one miRNA targeting both regions (\u22655 sites each)") +
  theme(plot.subtitle = element_text(size = 9, colour = "grey30"))

ggsave("../output/04.2-miRNA-comparison-targets-FE/paired_CDS_vs_UTR.png",
       fig3, width = 7.5, height = 3.8, dpi = 300, bg = "white")

fig1
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-35-1.png)<!-- -->

``` r
fig2
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-35-2.png)<!-- -->

``` r
fig3
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-35-3.png)<!-- -->
...
# Significance label formatter (returns NA for non-significant p-values)
sig_label <- function(p) {
  if (p >= 0.05) return(NA_character_)
  star <- dplyr::case_when(
    p < 0.001 ~ "***",
    p < 0.01  ~ "**",
    p < 0.05  ~ "*"
  )
  pp <- if (p < 0.001) format(p, scientific = TRUE, digits = 2) else sprintf("%.3f", p)
  paste0("p = ", pp, "  ", star)
}

# Helper to pull a p-value out of the lower-triangular matrix returned by
# pairwise.wilcox.test, regardless of factor-level order
get_pw <- function(mat, a, b) {
  if (a %in% rownames(mat) && b %in% colnames(mat) && !is.na(mat[a, b])) mat[a, b]
  else                                                                    mat[b, a]
}

# Build significance brackets for fig1/fig2, keeping only significant pairs
build_signif <- function(pw_mat, comps, y_positions) {
  annotations <- sapply(comps, function(c) sig_label(get_pw(pw_mat, c[1], c[2])))
  keep <- !is.na(annotations)
  if (sum(keep) == 0) return(NULL)
  list(
    comparisons = comps[keep],
    annotations = annotations[keep],
    y_position  = y_positions[keep]
  )
}
...
# Cross-species omnibus + pairwise tests
pw_CDS <- pairwise.wilcox.test(mir_props_filt$prop_CDS, mir_props_filt$species,
...
``` r
# Comparisons drawn on fig1 and fig2
comps <- list(c("Apul", "Peve"), c("Apul", "Ptuh"), c("Peve", "Ptuh"))

# Panel A: Region distribution (CDS vs UTR)

fig1 <- ggplot(mir_props_filt, aes(species, prop_CDS, fill = species)) +
  geom_boxplot(width = 0.55, outlier.shape = NA, alpha = 0.8,
               linewidth = 0.4) +
  geom_jitter(aes(colour = species),
              width = 0.18, height = 0, size = 1.6, alpha = 1) +
  # mean as a white-filled diamond
  ...
  scale_y_continuous(limits = c(0, 1.1),
                     ...
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of sites in CDS",
       title = "A. Region distribution: CDS vs UTR") +
  theme(axis.text.x = element_text(face = "italic"),
        legend.position = "none")

# Add significance brackets for significant comparisons only
signif_a <- build_signif(pw_CDS, comps, c(0.98, 1.05, 1.1))
if (!is.null(signif_a)) {
  fig1 <- fig1 +
    geom_signif(
      comparisons = signif_a$comparisons,
      annotations = signif_a$annotations,
      y_position  = signif_a$y_position,
      tip_length  = 0.012, textsize = 3, vjust = -0.2
    )
}

# Panel B: Coexpression direction across species
...
fig2 <- ggplot(mir_props, aes(species, prop_pos, fill = species)) +
  ...
  scale_y_continuous(limits = c(0, 1.1),
                     ...
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of positively-coexpressed sites",
       title = "B. Coexpression direction across species") +
  theme(axis.text.x = element_text(face = "italic"),
        legend.position = "none")

# Add significance brackets for significant comparisons only
signif_b <- build_signif(pw_pos, comps, c(0.98, 1.05, 1.1))
if (!is.null(signif_b)) {
  fig2 <- fig2 +
    geom_signif(
      comparisons = signif_b$comparisons,
      annotations = signif_b$annotations,
      y_position  = signif_b$y_position,
      tip_length  = 0.012, textsize = 3, vjust = -0.2
    )
}

# Panel C: Within-species paired CDS vs UTR comparison
# paired_data was built in the previous code chunk

# Per-species paired Wilcoxon
paired_wide <- paired_data %>%
  pivot_wider(id_cols     = c(species, given_miRNA_name),
              names_from  = region,
              values_from = prop_pos) %>%
  tidyr::drop_na(CDS, UTR)

paired_stats <- paired_wide %>%
  group_by(species) %>%
  summarise(
    p_val = wilcox.test(CDS, UTR, paired = TRUE)$p.value,
    .groups  = "drop"
  ) %>%
  mutate(sig_lab = vapply(p_val, sig_label, character(1))) %>%
  filter(!is.na(sig_lab) & sig_lab != "")

# Per-species per-region means for diamond annotation
paired_means_long <- paired_wide %>%
  group_by(species) %>%
  summarise(mean_UTR = mean(UTR), mean_CDS = mean(CDS), .groups = "drop") %>%
  pivot_longer(c(mean_UTR, mean_CDS), names_to = "region", values_to = "mean_pp") %>%
  mutate(region = factor(sub("mean_", "", region), levels = c("UTR", "CDS")))

# Bracket annotation data — manual = TRUE in geom_signif uses these as-is
# (only includes species with significant p-values)
signif_df <- if (nrow(paired_stats) > 0) {
  paired_stats %>%
    transmute(species,
              xmin = "UTR", xmax = "CDS",
              y_position  = 1.1,
...
fig3 <- ggplot(paired_data,
               aes(region, prop_pos, group = given_miRNA_name, colour = species, fill = species)) +
  # geom_hline(yintercept = 0.5, linetype = "dashed",
  #            colour = "grey55", linewidth = 0.4) +
  geom_line(alpha = 0.55, linewidth = 0.6) +
  geom_point(size = 1.7, alpha = 1, shape = 21) +
  # mean as a white-filled diamond per region within each facet
  geom_point(data = paired_means_long,
             aes(x = region, y = mean_pp),
             inherit.aes = FALSE,
             shape = 23, size = 3, stroke = 0.8,
             fill = "white", colour = "black") +
  facet_wrap(~ species, labeller = labeller(species = species_labels)) +
  ...
  scale_y_continuous(limits = c(0, 1.1),
                     ...
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of positively-coexpressed sites",
       title = "C. Within-species CDS vs UTR comparison") +
  theme(strip.text    = element_text(face = "italic"),
        legend.position = "bottom",
        legend.text = element_text(size = 9, face = "italic"))

# Add significance brackets for significant species only
...
```
    ...
    ## Warning in geom_signif(data = signif_df, aes(xmin = xmin, xmax = xmax,
    ## y_position = y_position, : Ignoring unknown aesthetics: xmin, xmax, y_position,
    ## and annotations
...
fig1 + fig2 + fig3 +
  plot_layout(ncol = 3, widths = c(1, 1, 1.4)) +
  plot_annotation(
    theme = theme(plot.background = element_rect(fill = "white", color = NA))
  )
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-36-1.png)<!-- -->

``` r
ggsave("../output/04.2-miRNA-comparison-targets-FE/region_direction_combined.png",
