...
Fancy version for manuscript:

``` r
# ---- HELPERS ----

# Significance label formatter
...
``` r
# Comparisons drawn on both fig1 and fig2
comps <- list(c("Apul", "Peve"), c("Apul", "Ptuh"), c("Peve", "Ptuh"))


# ---- PLOT 1 ----

fig1_fancy <- ggplot(mir_props_filt, aes(species, prop_CDS, fill = species)) +
  geom_hline(yintercept = 0.5, linetype = "dashed",
             colour = "grey55", linewidth = 0.4) +
  geom_boxplot(width = 0.55, outlier.shape = NA, alpha = 0.55,
               linewidth = 0.4) +
  ...
  geom_signif(
    comparisons = comps,
    annotations = c(sig_label(get_pw(pw_CDS, "Apul", "Peve")),
                    sig_label(get_pw(pw_CDS, "Apul", "Ptuh")),
                    sig_label(get_pw(pw_CDS, "Peve", "Ptuh"))),
    y_position  = c(1.08, 1.20, 1.32),
  ...
  scale_colour_manual(values = species_pal) +
  scale_x_discrete(labels = species_labels) +
  scale_y_continuous(limits = c(0, 1.4),                   # extended to fit brackets
                     breaks = seq(0, 1, 0.25),
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of sites in CDS",
       title = "Region distribution: CDS vs UTR") +
...
fig2_fancy <- ggplot(mir_props, aes(species, prop_pos, fill = species)) +
  geom_hline(yintercept = 0.5, linetype = "dashed",
             colour = "grey55", linewidth = 0.4) +
  geom_boxplot(width = 0.55, outlier.shape = NA, alpha = 0.55,
               linewidth = 0.4) +
  ...
  geom_signif(
    comparisons = comps,
    annotations = c(sig_label(get_pw(pw_pos, "Apul", "Peve")),
                    sig_label(get_pw(pw_pos, "Apul", "Ptuh")),
                    sig_label(get_pw(pw_pos, "Peve", "Ptuh"))),
    y_position  = c(1.08, 1.20, 1.32),
    tip_length  = 0.012, textsize = 3, vjust = -0.2
  ) +
...
# ---- PLOT 3 ----
# Per-species means & paired Wilcoxon
...
paired_stats <- paired_wide %>%
  ...
  mutate(sig_lab = vapply(p_val, sig_label, character(1)),
         n_lab   = sprintf("%d of %d miRNAs CDS > UTR", n_CDSgt, n))

# Long-format means table for geom_segment / geom_text
paired_means_long <- paired_stats %>%
  dplyr::select(species, mean_UTR, mean_CDS) %>%
  pivot_longer(c(mean_UTR, mean_CDS),
               names_to = "region", values_to = "mean_pp") %>%
...
fig3_fancy <- ggplot(paired_data,
  ...
  # mean crossbar per region within each facet
  geom_segment(data = paired_means_long,
               aes(x    = as.numeric(region) - 0.22,
                   xend = as.numeric(region) + 0.22,
                   y    = mean_pp, yend = mean_pp),
               inherit.aes = FALSE,
               colour = "black", linewidth = 0.9) +

  # # mean value as text just above the crossbar
  # geom_text(data = paired_means_long,
  #           aes(x = region, y = mean_pp,
  #               label = sprintf("%.2f", mean_pp)),
  #           inherit.aes = FALSE,
  #           vjust = -0.7, size = 3, fontface = "bold", colour = "grey20") +

  # per-facet paired Wilcoxon bracket
  ...
  # # small "X of N miRNAs" note above the bracket
  # geom_text(data = paired_stats,
  #           aes(x = 1.5, y = 1.18, label = n_lab),
  #           inherit.aes = FALSE,
  #           size = 2.8, fontface = "italic", colour = "grey35") +

  facet_wrap(~ species, labeller = labeller(species = species_labels)) +
  scale_colour_manual(values = species_pal) +
  scale_y_continuous(limits = c(0, 1.1),                   # extended to fit bracket
                     breaks = seq(0, 1, 0.25),
                     labels = scales::percent_format(accuracy = 1)) +
  labs(x = NULL, y = "Per-miRNA proportion of positively-coexpressed sites",
       title    = "Within-species CDS vs UTR comparison") +
...
![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-36-2.png)<!-- -->

``` r
fig3_fancy
```

![](04.2-miRNA-comparison-targets-FE_files/figure-gfm/unnamed-chunk-36-3.png)<!-- -->

``` r
ggsave("../output/04.2-miRNA-comparison-targets-FE/region_distribution_fancy.png", fig1_fancy, width = 3.5, height = 4.6, dpi = 300, bg = "white")
ggsave("../output/04.2-miRNA-comparison-targets-FE/direction_fancy.png", fig2_fancy, width = 3.5, height = 4.6, dpi = 300, bg = "white")
ggsave("../output/04.2-miRNA-comparison-targets-FE/paired_CDS_vs_UTR_fancy.png", fig3_fancy, width = 5, height = 4.6, dpi = 300, bg = "white")
```
