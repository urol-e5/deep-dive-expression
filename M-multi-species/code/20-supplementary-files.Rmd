...
# WGBS methylation summary stats

Per-sample bisulfite sequencing methylation statistics for all three species, from the Bismark PE alignment reports (`*_PE_report.txt`) in each species' `bismark_cutadapt/` output directory.

```{r}
# Parse a single Bismark PE report for mapping efficiency and methylation calls
parse_bismark_report <- function(path, species_code) {
  lines <- readLines(path)

  # Extract sample name from filename, e.g. trimmed_ACR-140-TP2_S5_PE_report.txt
  fname <- basename(path)
  sample <- str_extract(fname, "(ACR|POR|POC)-\\d+")

  # Mapping efficiency
  mapping_eff <- as.numeric(
    str_match(lines[str_detect(lines, "^Mapping efficiency:")],
              "^Mapping efficiency:\\t(\\S+)%")[2]
  )

  # C methylated in each C context (CpG, CHG, CHH)
  pct <- function(context) {
    pat <- paste0("^C methylated in ", context, ":\\t(\\S+)%")
    as.numeric(str_match(lines[str_detect(lines, pat)], pat)[2])
  }
  ...
  tibble(
    ...
    pct_meth_CpG        = pct("CpG context"),
    pct_meth_CHG        = pct("CHG context"),
    pct_meth_CHH        = pct("CHH context"),
    pct_meth_unknown    = pct("unknown context \\(CN or CHN\\)")
  )
}

# Per-species report directories
wgbs_report_dirs <- list(
  "D-Apul" = "../../D-Apul/output/08-Apul-WGBS/bismark_cutadapt",
  "E-Peve" = "../../E-Peve/output/12-Peve-WGBS/bismark_cutadapt",
  "F-Ptuh" = "../../F-Ptuh/output/12-Ptuh-WGBS/bismark_cutadapt"
)

methylation_stats <- bind_rows(lapply(names(wgbs_report_dirs), function(sp) {
  files <- list.files(wgbs_report_dirs[[sp]], pattern = "_PE_report\\.txt$",
                      full.names = TRUE)
  bind_rows(lapply(files, parse_bismark_report, species_code = sp))
}))

methylation_stats

write.csv(methylation_stats, "../output/20-supplementary-files/WGBS_methylation_stats.csv", row.names = FALSE)
```

Per-species means across samples
```{r}
methylation_summary <- methylation_stats %>%
  group_by(species) %>%
  summarise(
    mean_mapping_efficiency = round(mean(mapping_efficiency), 2),
    mean_pct_meth_CpG       = round(mean(pct_meth_CpG), 2),
    mean_pct_meth_CHG       = round(mean(pct_meth_CHG), 2),
    mean_pct_meth_CHH       = round(mean(pct_meth_CHH), 2),
    mean_pct_meth_unknown   = round(mean(pct_meth_unknown), 2),
    .groups = "drop"
  )

methylation_summary

write.csv(methylation_summary,
          "../output/20-supplementary-files/WGBS_methylation_summary.csv",
          row.names = FALSE)
```
...
miRNA_names_apul <-miRNA_details %>% dplyr::select(Name, given_miRNA_name) %>% filter(grepl("apul", given_miRNA_name))
miRNA_names_peve <-miRNA_details %>% dplyr::select(Name, given_miRNA_name) %>% filter(grepl("peve", given_miRNA_name))
miRNA_names_ptuh <-miRNA_details %>% dplyr::select(Name, given_miRNA_name) %>% filter(grepl("ptuh", given_miRNA_name))
...
# Combine all 3 regions for each species
...
apul_miRanda_PCC_mRNA <- left_join(apul_miRanda_PCC_mRNA, miRNA_names_apul, by = c("miRNA" = "Name"))
peve_miRanda_PCC_mRNA <- left_join(peve_miRanda_PCC_mRNA, miRNA_names_peve, by = c("miRNA" = "Name"))
ptuh_miRanda_PCC_mRNA <- left_join(ptuh_miRanda_PCC_mRNA, miRNA_names_ptuh, by = c("miRNA" = "Name"))
...
# Annotate with given miRNA names
apul_miRanda_PCC_lncRNA <- left_join(apul_miRanda_PCC_lncRNA, miRNA_names_apul, by = c("miRNA" = "Name"))
peve_miRanda_PCC_lncRNA <- left_join(peve_miRanda_PCC_lncRNA, miRNA_names_peve, by = c("miRNA" = "Name"))
ptuh_miRanda_PCC_lncRNA <- left_join(ptuh_miRanda_PCC_lncRNA, miRNA_names_ptuh, by = c("miRNA" = "Name"))
...
miRanda_PCC_lncRNA <- miRanda_PCC_lncRNA %>% dplyr::select(miRNA, lncRNA, PCC.cor, p_value, adjusted_p_value, score, energy, query_start_end, subject_start_end, total_bp_shared, query_similar, subject_similar, given_miRNA_name)
...
```{r}
# Read in ceRNA triad files
# These contain the triad members (miRNA, mRNA, and lncRNA) and the mRNA-lncRNA coexpression stats
...
# Join to include miRNA-mRNA coexpression stats
miRNA_mRNA <- miRanda_PCC_mRNA_sig %>% dplyr::select(given_miRNA_name, mRNA, PCC.cor, p_value, adjusted_p_value) %>% rename(PCC_miRNA_mRNA = PCC.cor, p_value_miRNA_mRNA = p_value, FDR_miRNA_mRNA = adjusted_p_value) %>% unique()

apul_ceRNA_plus <- left_join(apul_ceRNA, miRNA_mRNA, by = c("miRNA" = "given_miRNA_name", "mRNA" = "mRNA"))
peve_ceRNA_plus <- left_join(peve_ceRNA, miRNA_mRNA, by = c("miRNA" = "given_miRNA_name", "mRNA" = "mRNA"))
ptuh_ceRNA_plus <- left_join(ptuh_ceRNA, miRNA_mRNA, by = c("miRNA" = "given_miRNA_name", "mRNA" = "mRNA"))


# Join to include miRNA-lncRNA coexpression stats
miRNA_lncRNA <- miRanda_PCC_lncRNA_sig %>% dplyr::select(given_miRNA_name, lncRNA, PCC.cor, p_value, adjusted_p_value) %>% rename(PCC_miRNA_lncRNA = PCC.cor, p_value_miRNA_lncRNA = p_value, FDR_miRNA_lncRNA = adjusted_p_value) %>% unique()

apul_ceRNA_plus2 <- left_join(apul_ceRNA_plus, miRNA_lncRNA, by = c("miRNA" = "given_miRNA_name", "lncRNA" = "lncRNA"))
peve_ceRNA_plus2 <- left_join(peve_ceRNA_plus, miRNA_lncRNA, by = c("miRNA" = "given_miRNA_name", "lncRNA" = "lncRNA"))
ptuh_ceRNA_plus2 <- left_join(ptuh_ceRNA_plus, miRNA_lncRNA, by = c("miRNA" = "given_miRNA_name", "lncRNA" = "lncRNA"))

# Give species labels
apul_ceRNA_plus2$species <- "A. pulchra"
peve_ceRNA_plus2$species <- "P. evermanni"
ptuh_ceRNA_plus2$species <- "P. tuahiniensis"

# Combine the three species
ceRNA_triads_PCC <- rbind(apul_ceRNA_plus2, peve_ceRNA_plus2, ptuh_ceRNA_plus2)

# Order so that first column is species ID
ceRNA_triads_PCC <- ceRNA_triads_PCC %>% dplyr::select(species, colnames(ceRNA_triads_PCC %>% dplyr::select(-species)))

# Save
write.csv(ceRNA_triads_PCC, "../output/20-supplementary-files/ceRNA_triads_PCC.csv", row.names = FALSE)
```
