...
-p 32 \
-eB \
-G ../data/Porites_evermanni_validated.gff3 \
-o ../output/06.2-Peve-Hisat/{}.gtf \
../output/06-Peve-Hisat/{}.sorted.bam
```


```{r, engine='bash', eval=TRUE}
wc -l ../output/06.2-Peve-Hisat/RNA*.gtf
ls ../output/06.2-Peve-Hisat/RNA*.gtf

```
```{bash}
head -3 ../output/06.2-Peve-Hisat/RNA*.gtf
```
...
```{bash}
for file in $(ls ../output/06.2-Peve-Hisat/*.gtf); do
  sample=$(basename "$file" .gtf)  # Extract sample name from filename
  echo "$sample $file"
done > ../output/06.2-Peve-Hisat/list01.txt
```
...
```{r, engine='bash'}
head ../output/06.2-Peve-Hisat/list01.txt
```
...
-i ../output/06.2-Peve-Hisat/list01.txt \
-g ../output/06.2-Peve-Hisat/gene_count_matrix.csv \
-t ../output/06.2-Peve-Hisat/transcript_count_matrix.csv
...
```{bash}
cp ../output/06.2-Peve-Hisat/gene_count_matrix.csv ../output/06.2-Peve-Hisat/Peve-gene_count_matrix.csv
```
