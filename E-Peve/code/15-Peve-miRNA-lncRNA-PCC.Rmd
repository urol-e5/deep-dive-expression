...
```{r}
lncRNA_counts<-read_table(file="../../M-multi-species/output/Peve_counts.txt", skip=1) %>%
  rename("lncrna_id"=Geneid, 
         "sample71"=`../output/01.61-lncRNA-pipeline/RNA-POR-71-S1-TP2.sorted.bam`, 
         "sample73"=`../output/01.61-lncRNA-pipeline/RNA-POR-73-S1-TP2.sorted.bam`, 
         "sample76"=`../output/01.61-lncRNA-pipeline/RNA-POR-76-S1-TP2.sorted.bam`, 
         "sample79"=`../output/01.61-lncRNA-pipeline/RNA-POR-79-S1-TP2.sorted.bam`, 
         "sample82"=`../output/01.61-lncRNA-pipeline/RNA-POR-82-S1-TP2.sorted.bam`)
...
lncRNA_mapping <- data.frame(
  lncRNA_id = lncRNA_counts_filt$lncrna_id,
  lncRNA_coord = paste0(lncRNA_counts_filt$Chr, ":", lncRNA_counts_filt$Start, "-", lncRNA_counts_filt$End)
)

# Save for future use
write.table(lncRNA_mapping, "../output/15-Peve-miRNA-lncRNA-PCC/Peve_lncRNA_mapping.tab")

# Seems to be a duplicates issue (multiple unique lncRNA ids represent the same coordinates/counts)
# May only be some instances of all-zero counts, so reduce this mapping to only keep lncRNA included in the PCC calc
lncRNA_mapping_filt <- lncRNA_mapping[lncRNA_mapping$lncRNA_id %in% rownames(lncRNA_counts_df),]
```
...
```{r}
miranda_peve_names <- left_join(miranda_peve, lncRNA_mapping_filt, by = c("lncRNA" = "lncRNA_coord")) %>%
...
```{r}
pcc_miranda_peve <- left_join(miranda_peve_names, pcc_results, by = c("miRNA", "lncRNA_id" = "lncRNA"))
...
For the significant pairs, the miRNAs can interact with 3-128 unique lncRNAs, while the lncRNAs can interact with with 1-4 unique miRNAs. For the significant pairs that have high query similarity, the miRNAs can interact with 1-68 unique lncRNAs, while the lncRNAs can interact with 1-3 unique miRNAs. Interesting! 
