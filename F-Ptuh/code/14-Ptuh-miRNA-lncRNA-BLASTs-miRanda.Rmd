---
title: "14-Ptuh-miRNA-lncRNA-BLASTs-miRanda"
...
LncRNAs were identified from Ptuh RNA-seq data in `deep-dive/main/F-Pmea/output/02-lncRNA-discovery` -- see details there.
Fasta of Ptuh lncRNAs stored at `https://raw.githubusercontent.com/urol-e5/deep-dive/main/F-Pmea/output/02-lncRNA-discovery/Pmea_lncRNA.fasta`

(Note the original file is named Pmea beacuse, at the time it was generated, we believed our samples were P. meandrina. Genetic analysis later confirmed the samples are actually P.tuahiniensis, thus the renaming)

```{r check-lncRNAs, engine='bash', eval=TRUE}
curl -L https://raw.githubusercontent.com/urol-e5/deep-dive/main/F-Pmea/output/02-lncRNA-discovery/Pmea_lncRNA.fasta -o ../output/14-Ptuh-miRNA-lncRNA-BLASTs-miRanda/Ptuh_lncRNA.fasta
...
Generate a list of blast results. It seems plausible that a single lncRNA, which would be hundreds or thousands of nucleotides long, could interact with multiple miRNAs, so I will allow up to 10 hits (~25% of Ptuh miRNAs) for each lncRNA. I want to see the top hits no matter how poor the match is, so I will not filter by e-value at this stage. I’ll also include the “-word_size 4” option, which reduces the required length of the initial match.
...
```
We have 0 alignments of a full pre-miRNA to a lncRNA with no mismatches.
...
```
44 putative lncRNA sponges with these parameters. 
...
This is a dramatically smaller number -- only  interactions are at least 21 nucleotides with <=3 mismatches