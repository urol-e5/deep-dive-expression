...
LncRNAs were identified from Ptuh RNA-seq data -- see details in `F-Ptuh/code/17-Ptuh-lncRNA.Rmd`

Fasta of Ptuh lncRNAs stored at `https://raw.githubusercontent.com/urol-e5/deep-dive-expression/refs/heads/main/F-Ptuh/output/17-Ptuh-lncRNA/Ptuh-lncRNA.fasta`
...
precursor_lncRNA_BLASTn %>% 
  ...
  unique()
```
We have 8 alignments of a full pre-miRNA to a lncRNA with no mismatches. 8 lncRNA and 2 miRNA are represented. 

Note that, as in A.pulchra and P.evermanni, there are instances of a single pre-miRNA matching to several overlapping lncRNA, potentially representing multiple isoforms of a single lncRNA gene.
...
This is a dramatically smaller number -- only 9 interactions are at least 115 nucleotides with <=3 mismatches
