...
```{r}
miRNAs = genomation::gffToGRanges("../output/05-Ptuh-sRNA-ShortStack_4.1.0/ShortStack_out/known_miRNAs.gff3")
head(miRNAs)
```
#### lncRNA:

```{r}
lncRNAs = genomation::gffToGRanges("../output/08-Ptuh-lncRNA-matrix/Peve-lncRNAs.gtf")
head(lncRNAs)
```
...
#all_annotated <- GenomicRanges::reduce(c(gff5UTR, gff3UTR, exons, introns, TE))
all_annotated <- GenomicRanges::reduce(c(exons, introns, TE))
...
df_annotated$region <- factor(df_annotated$region,
                              levels = c("intergenic", "TE"," 3UTR", "5UTR", "intron", "exon"))

blue_palette <- c("intergenic" = "#e9edf0", "TE"= "#c6dbef", "3UTR" = "#9ecae1","5UTR" = "#6baed6","intron"= "#3182bd","exon" = "#08519c"   )
...
df_annotated$meth_status <- cut(df_annotated$avg_meth,
                                breaks = c(0, low_thresh, high_thresh, 100),
