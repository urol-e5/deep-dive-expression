...
Both matrices (the mRNA counts and lncRNA counts) are prohibitively large (~30,000 rows). I've used the above code to create an R script that will perform the PCC calculations, `run_mRNA_lncRNA_PCC.R` stored in `D-Apul/output/21.2-Apul-mRNA-lncRNA-correlation-PCC`. To run the PCC calcs in the background, allowing continued work in Rstudio, run the below code chunk from the terminal.
...
33,624 mRNA significantly correlate (p-val < 0.05) with a total of 26,416 lncRNA, with a total of 96,470,124 unique pairs with significantly correlated expression. 

Of these, all also had correlation magnitudes of at least 0.5. The majority of correlations (87,393,834 or 90.6%) were positive, though some (9,076,290 or 9.4%) were negative.
...
For the significant pairs (|PCC| > 0.5, pval < 0.05), the mRNAs are correlated with 290-10,692 unique lncRNAs, while the lncRNAs are correlated with 1-7,695 unique mRNAs. 
