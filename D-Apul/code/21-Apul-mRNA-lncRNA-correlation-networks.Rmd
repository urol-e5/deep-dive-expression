...
apul_cor_results <- corr.test(t(vsd_mrna), t(vsd_lncrna), method = "spearman", adjust = "BH")
...
apul_significant_edges <- which(abs(apul_cor_matrix) > apul_cor_threshold & p_matrix < p_threshold, arr.ind = TRUE)
...
# Assign node types for visualization (mRNA vs. lncRNA)
V(apul_network)$type <- ifelse(V(apul_network)$name %in% rownames(vsd_mrna), "mRNA", "lncRNA")
```
...
ggraph(apul_network, layout = "fr") +  # Force-directed layout
  geom_edge_link(aes(edge_alpha = abs(correlation)), show.legend = FALSE) +
  geom_node_point(aes(color = type), size = 4) +
  geom_node_text(aes(label = name), repel = TRUE, size = 3) +
  theme_void() +
  labs(title = "A. pulchra mRNA-lncRNA Correlation Network")
```
...
gene_module_info$Gene <- paste0("ME", gene_module_info$Gene)
...
ggraph(apul_network, layout = "fr") +  # Force-directed layout
  geom_edge_link(aes(edge_alpha = abs(correlation)), show.legend = FALSE) +
  geom_node_point(aes(colour=prop_mrna), size = 4) +
  geom_node_text(aes(label = name), repel = TRUE, size = 3) +
  theme_void() +
  labs(title = "A. pulchra WGCNA Network")
```
...
ggplot(stack_long, aes(x = module, y = Count, fill = RNA_Type)) +
  ...
  theme_classic() +
  labs(title = "Counts of mRNA and lncRNA in Each Module",
       x = "Module",
       y = "Counts",
       fill = "RNA Type") +
...
ggcorrplot(apul_cor_results$r, 
           ...
           title = "Module Correlation Matrix with p-values")
```
